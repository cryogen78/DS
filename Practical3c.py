...
class LinkedList:
    ...
    def remove_tail(self):
        if self.is_empty():
            ...
            Node_5 = self.find_second_last_element()
            if Node:
                Node_5.next = None
                self.size -= 1
...
my_list.add_head(Node_5(int(input(f"Enter coefficient for power {order} : "))))
for i in reversed(range(order)):
    my_list.add_tail(int(input(f"Enter coefficient for power {i} : ")))
    
my_list2 = LinkedList()
my_list2.add_head(Node_5(int(input(f"Enter coefficient for power {order} : "))))
for i in reversed(range(order)):
